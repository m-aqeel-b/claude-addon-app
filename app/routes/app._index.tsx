...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  return { bundles, stats, statusFilter, searchQuery, shop };
};
...
export default function BundleList() {
  const { bundles, stats, statusFilter, searchQuery, shop } = useLoaderData<LoaderData>();
  ...
  // Handler to open theme editor for App Embeds
  const handleOpenAppEmbeds = useCallback(() => {
    // Opens the theme editor's App Embeds section
    // This allows merchants to enable/disable the app embed globally
    const themeEditorUrl = `https://${shop}/admin/themes/current/editor?context=apps`;
    window.open(themeEditorUrl, "_blank");
  }, [shop]);

  // Handler to open theme editor for adding Product Page Block
  const handleOpenProductPageEditor = useCallback(() => {
    // Opens the theme editor on product template with the app block ready to add
    // The addAppBlockId format is: {app_client_id}/{block_key}
    // App client_id from shopify.app.toml: 3136e167c35c9827dd2803de3deecc60
    // Block key from extension's shopify.extension.toml: addon-bundle
    // Note: The JavaScript auto-positions the widget near buy buttons after page load
    const appClientId = "3136e167c35c9827dd2803de3deecc60";
    const blockKey = "addon-bundle";
    // Target the main-product section for better initial placement
    const themeEditorUrl = `https://${shop}/admin/themes/current/editor?template=product&addAppBlockId=${appClientId}/${blockKey}`;
    window.open(themeEditorUrl, "_blank");
  }, [shop]);
  ...
  const widgetSetupSectionStyle: React.CSSProperties = {
  ...
  const widgetSetupHeaderStyle: React.CSSProperties = {
    display: "flex",
    alignItems: "center",
    gap: "12px",
    marginBottom: "20px",
  ...
  const widgetSetupIconStyle: React.CSSProperties = {
    width: "44px",
    height: "44px",
    borderRadius: "12px",
    backgroundColor: "#008060",
  ...
  const widgetSetupCardsStyle: React.CSSProperties = {
    display: "flex",
    gap: "20px",
    flexWrap: "wrap",
  };
  ...
  return (
    <s-page heading="Add-On Bundles">
      ...
      <div style={pageContainerStyle}>
        ...
        <div style={widgetSetupSectionStyle}>
          <div style={widgetSetupHeaderStyle}>
            <div style={widgetSetupIconStyle}>
            ...
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 600, color: "#202223", margin: "0 0 4px" }}>
                Widget Setup
              </h2>
              <p style={{ fontSize: "14px", color: "#6d7175", margin: 0 }}>
                Enable the widget on your storefront to start showing add-on bundles
              </p>
            </div>
          </div>
          <div style={widgetSetupCardsStyle}>
            <WidgetSetupCard
              title="App Embed"
              description="Enable the app embed in your theme settings to load the widget scripts globally. This is required for the widget to function properly on your store."
              buttonText="Enable in Theme Editor"
              onClick={handleOpenAppEmbeds}
              icon={
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#5c6ac4" strokeWidth="2">
                  <rect x="3" y="3" width="18" height="18" rx="2" ry="2" />
                  <line x1="3" y1="9" x2="21" y2="9" />
                  <line x1="9" y1="21" x2="9" y2="9" />
                </svg>
            ...
            />
            <WidgetSetupCard
              title="Product Page Element"
              description="Add the widget block to your product page. The widget automatically positions itself near the Add to Cart button for optimal conversion."
              buttonText="Add to Product Page"
              onClick={handleOpenProductPageEditor}
              icon={
